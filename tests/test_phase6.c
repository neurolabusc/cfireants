...
int main(int argc, char **argv) {
    ...
    if (image_load(&fixed, "validate/small/MNI152_T1_2mm.nii.gz", DEVICE_CPU) != 0 ||
        ...
        fprintf(stderr, "cannot load validate/small -- run from repo root\n");
        return 1;
    }
    ...
    if (moments_register(&fixed, &moving, mom_opts, &mom_result) != 0) return 1;
    ...
    if (rigid_register(&fixed, &moving, &mom_result, ropts, &rigid_result) != 0) return 1;
    ...
    if (affine_register(&fixed, &moving, rigid_result.rigid_mat, aopts, &affine_result) != 0) return 1;
    ...
    if (greedy_register(&fixed, &moving, aff44, gopts, &greedy_result) != 0) return 1;
    printf("  Greedy NCC:  %.6f\n", greedy_result.ncc_loss);

    /* Load Python SyN reference for comparison baseline */
    float py_ncc;
    if (load_bin_f32(data_dir, "syn_small_ncc", &py_ncc, 1) != 0) return 1;

...
fn adam_moments_update(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>) {
...
@compute @workgroup_size(256)
fn adam_direction(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>) {
    let i = gid.x + gid.y * nwg.x * 256u;
    if (i >= dp.n) { return; }
...
fn max_l2_norm(@builtin(global_invocation_id) gid: vec3<u32>,
    ...
    if (tid == 0u) { norm_out[wid.x + wid.y * nwg.x] = smax[0]; }
}
